...
int main(int argc, char **argv) {
    ...
    MPI_Offset start, count=1, bufcount;
    ...
    else          strcpy(filename, "/Users/liangweihao/Desktop/testfile.nc");
    ...
    ret = ncmpi_def_var(ncfile, "v1", NC_BYTE, ndims, &dimid, &varid1);
    if (ret != NC_NOERR) handle_error(ret, __LINE__);
   ...
   // ret = ncmpi_put_vara_int_all(ncfile, varid1, &start, &count, &data);
    //if (ret != NC_NOERR) handle_error(ret, __LINE__);
    bufcount = 1;
    ret = ncmpi_put_vara_all(ncfile, varid1, &start, &count, &cdata, bufcount, MPI_SIGNED_CHAR);
    if (ret != NC_NOERR) handle_error(ret, __LINE__);
